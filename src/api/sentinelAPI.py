"""
    ...
    Args:
        ...
        filter_tile_pair: Liste de tuiles requises ensemble (ex: ['T20TNT', 'T20TPT'])
...
def authenticate_google_drive():
    ...
    6. Placer credentials.json dans le même dossier que ce script
...
def upload_stream_to_drive(service, file_stream, filename, folder_id, mime_type='application/zip'):
    """
    Uploader un flux de données vers Google Drive
...
def upload_best_pairs_to_drive(best_pairs, username, password, base_folder_name='Sentinel-2'):
    ...
    try:
        ...
        print("3. Vous avez autorise l'application")
        return
    ...
    # Compter le total
    total_images = sum(len(images) for images in best_pairs.values() if images)
    print(f"\nTotal d'images a uploader: {total_images}")
    ...
    for year in sorted(best_pairs.keys()):
        ...
        print(f"\n{'='*80}")
        print(f"ANNEE {year} - {len(images)} image(s)")
        print(f"{'='*80}")
        ...
        for i, img in enumerate(images, 1):
            ...
            print(f"\n[{uploaded + failed + 1}/{total_images}] Image {i}/{len(images)} de {year}")
...
if __name__ == "__main__":
    ...
    max_tide_time_diff = 60  # minutes de différence max entre image et marée
    time_window_hours = 1  # heures de fenêtre de recherche autour des dates CSV
    
    # =========================================================================
    # MÉTHODE 1: RECHERCHE BASÉE SUR LES DATES DU CSV (NOUVEAU)
    # =========================================================================
    # Recherche UNIQUEMENT les images Sentinel-2 aux moments présents dans le CSV
    
    print("\n" + "="*80)
    print("MÉTHODE 1: RECHERCHE BASÉE SUR LE CSV")
    print("="*80)
    
    products_from_csv = search_sentinel2_from_csv_dates(
        username=username,
        password=password,
        ...
        use_api_complement=False,
        filter_tile_pair=['T20TNT', 'T20TPT']  # Garde uniquement les couples complets
    )
    
    # =========================================================================
    # MÉTHODE 2: RECHERCHE PAR PÉRIODE (ANCIEN)
    # =========================================================================
    # Recherche TOUTES les images dans une période, puis enrichit avec marée
    """
    print("\n" + "="*80)
    print("MÉTHODE 2: RECHERCHE PAR PÉRIODE")
    print("="*80)
    
    start_date = "20160101"
    end_date = "20251004"
    
    products_by_period = search_sentinel2_with_tides_csv(
        username=username,
        password=password,
        ...
        csv_file_path=csv_file,
        max_cloud_cover=max_cloud_cover,
        max_tide_time_diff=max_tide_time_diff,
        date_column='A',
        tide_column='B',
        csv_delimiter=';',
        use_both=True,
        api_station_id="5cebf1e33d0f4a073c4bc285"
    )
    """
    ...
    if products:
        ...
        """
        print("\n" + "="*80)
        print("ANALYSE DES MARÉES:")
        print("="*80)
        
        
        extremes = find_extreme_tides(products, max_time_diff_minutes=max_tide_time_diff)
        
        if extremes['high_tide']:
            ht = extremes['high_tide']
            print(f"\n🔼 IMAGE À MARÉE HAUTE:")
            print(f"   Niveau: {ht['tide_level_m']:.3f} m")
            print(f"   Capture: {ht['capture_datetime'].strftime('%Y-%m-%d %H:%M')}")
            print(f"   Différence: {ht['time_diff_minutes']:.1f} min")
        
        if extremes['low_tide']:
            lt = extremes['low_tide']
            print(f"\n🔽 IMAGE À MARÉE BASSE:")
            print(f"   Niveau: {lt['tide_level_m']:.3f} m")
            print(f"   Capture: {lt['capture_datetime'].strftime('%Y-%m-%d %H:%M')}")
            print(f"   Différence: {lt['time_diff_minutes']:.1f} min")
        """
        ...
        high = filter_by_tide_level(products, min_tide=1.5, max_time_diff_minutes=max_tide_time_diff)
        low = filter_by_tide_level(products, max_tide=0.5, max_time_diff_minutes=max_tide_time_diff)
        
        print(f"\n🌊 Images à marée haute (≥1.5m): {len(high)}")
        print(f"🌊 Images à marée basse (≤0.5m): {len(low)}")
        
        """

        # EXPORT JSON
        """
        with open('images_avec_marees.json', 'w', encoding='utf-8') as f:
            json.dump(products, f, indent=2, ensure_ascii=False, default=str)
        print(f"\n💾 Données exportées dans 'images_avec_marees.json'")
        """

        # SÉLECTION DES MEILLEURES PAIRES PAR ANNÉE
        best_pairs = select_best_pairs_per_year(
            products, 
            required_tiles=['T20TNT', 'T20TPT']
        )
        
        # Option 1: Télécharger les quicklooks uniquement
        # download_best_pairs(best_pairs, username, password, download_type='quicklook')
        
        # Option 2: Télécharger les images complètes (commenté par défaut)
        # download_best_pairs(best_pairs, username, password, download_type='full')
        
        # Exporter les meilleures paires en JSON
        flat_best = []
        for year, images in best_pairs.items():
            for img in images:
                img_copy = img.copy()
                img_copy['selected_year'] = year
                flat_best.append(img_copy)
        
        with open('meilleures_paires.json', 'w', encoding='utf-8') as f:
            json.dump(flat_best, f, indent=2, ensure_ascii=False, default=str)
        print(f"\n💾 Meilleures paires exportées dans 'meilleures_paires.json'")
        ...
        upload_best_pairs_to_drive(
            ...
            username, 
            password,
            base_folder_name='Sentinel-2_Iles_Madeleine'
        )

        # MENU INTERACTIF
        """
        print("\n" + "="*80)
        interactive_download_menu(products, username, password)
        """
    else:
        print("\n⚠️  Aucune image trouvée pour cette période.")